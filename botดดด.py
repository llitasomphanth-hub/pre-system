...
def _wallet_fail_text(result: dict) -> str:
    ...
    error_words = ["error", "failed", "timeout", "request failed", "invalid json"]
...
LATEST_EXPIRE_SEC = 12 * 60 * 60  # 12 ชั่วโมง
...
async def _build_balance_snapshot(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ...
    latest_text = str(latest.get("latest_text", "") or "")
    latest_updated_at = latest.get("updated_at")

    # สถานะหลักของบัญชี (ห้ามให้ latest มาทับ)
    if joined_state:
        balance = max(int(total_paid) - 789, 0)
        status = "เข้ากลุ่มสำเร็จ"
    elif total_paid > 0 or payment_sent_state:
        balance = max(int(total_paid), 0)
        status = "รอการเข้ากลุ่ม"
    else:
        balance = 0
        status = "ไม่พบรายการ"

    # latest แยกคนละชั้นจาก status หลัก และให้หายเองเมื่อเกินเวลา
    latest_status = ""
    ...
    if latest_kind == "failed" and latest_is_fresh:
        latest_status = "❌ จ่ายไม่สำเร็จ"
        if status == "ไม่พบรายการ":
            status = "ตรวจไม่สำเร็จ"
            balance = 0
            latest_status = ""
...
def build_balance_text(balance, status, latest_status="", update_time=None):
    ...
    if latest_status:
        ...
        if str(status) == "เข้ากลุ่มสำเร็จ":
            status_line = "✅ เข้ากลุ่มสำเร็จ"
        elif str(status) == "รอการเข้ากลุ่ม":
            status_line = "⚠️ รอการเข้ากลุ่ม"
        elif str(status) == "ตรวจไม่สำเร็จ":
            status_line = "❌ ตรวจไม่สำเร็จ"
        elif str(status) == "ไม่พบรายการ":
            status_line = "ไม่พบรายการ"
        else:
            status_line = str(status)
    ...
    if latest_status:
        return (
            "💰 Blance: " + str(balance) + "\n"
            "💳 Status: " + status_line + "\n"
            "♻️ Latest: " + str(latest_status) + "\n\n"
            "🕒 update: " + str(update_time)
        )
...
async def on_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu:payment":
        ...
        keyboard = _menu_back_keyboard([[InlineKeyboardButton("🔎 ยอดเงินคงเหลือ", callback_data="menu:check_balance")]])
        try:
...
def _wallet_fail_text(result: dict) -> str:
    ...
    expired_words = [
        "expired", "หมดอายุ", "หมดอายุแล้ว"
    ]
    invalid_words = [
        "invalid", "not found", "notfound", "ไม่พบ", "ลิ้งไม่ถูกต้อง", "link not found"
    ]
    error_words = [
        "error", "failed", "timeout", "request failed"
    ]
...
def _wallet_result_text(total_paid: float) -> str:
    remaining = max(0, int(VIP_MIN_AMOUNT - total_paid))
...
async def _process_wallet_link(update: Update, context: ContextTypes.DEFAULT_TYPE, msg, url: str, sender: str, sender_id: str, skip_processing: bool = False, seq: int | None = None):
    ...
    if _wallet_is_success(result):
        ...
        if total_paid >= VIP_MIN_AMOUNT:
            ...
            await _send_result(f"✅ {invite_link}", disable_web_page_preview=True)
        else:
            remaining = max(0, int(VIP_MIN_AMOUNT - total_paid))
            await _send_result(
                "❕ ตรวจพบยอดเงินไม่พอค่ะ\n"
    ...
    await _send_result(_wallet_fail_text(result))
...
async def payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _clear_payment_ui(context, chat_id)
    caption = PAY_HELP_TEXT
    keyboard = _menu_back_keyboard([[InlineKeyboardButton("🔎 เช็คยอดเงินคงเหลือ", callback_data="menu:check_balance")]])
    try:
        if os.path.exists(PAY_IMAGE_PATH):
            with open(PAY_IMAGE_PATH, "rb") as f:
                media_msg = await context.bot.send_photo(
...
async def on_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if spam_hit.matched and not decoded and not caption_url:
        ...
        await context.bot.send_message(
            chat_id=msg.chat_id,
            text=(
                "❌ไม่พบ QR code รบกวนส่งรูปที่มีคิวอาร์โค้ด/ภาพชัดๆ อีกครั้งนะคะ\n\n"
                "หากต้องการสอบถาม สามารถกดปุ่มแจ้งปัญหาได้ค่ะ"
            )
        )
        return
        ...
        if _wallet_is_success(result):
            ...
            if amount >= VIP_MIN_AMOUNT:
                ...
                await update.message.reply_text(
                    ...
                    f"💰 Balance: {int(amount)}/{VIP_MIN_AMOUNT}"
                )
        else:
