...
def _classify(text: Optional[str], msg_type: Optional[str], direction: Optional[str] = None):
    ...
    if "callback" in mtype or lower.startswith("button:"):
...
def _normalize_button_label(text: Optional[str]) -> str:
    raw = (text or "").replace("\n", " ").strip()
    raw = re.sub(r"\s+", " ", raw)
    if raw.lower().startswith("button:"):
        raw = raw.split(":", 1)[1].strip()
    raw = re.sub(r"\s*\[[^\]]+\]\s*$", "", raw).strip()
    return raw[:80]


def _looks_like_button_press(text: Optional[str], msg_type: Optional[str], direction: Optional[str]) -> bool:
    raw = (text or "").strip()
    lower = raw.lower()
    mtype = (msg_type or "").lower()
    direction = (direction or "").lower()

    if direction == "out" or not raw:
        return False
    if lower.startswith("/"):
        return False
    if TG_LINK_RE.search(raw) or TM_LINK_RE.search(raw):
        return False

    button_types = ("button", "callback", "click", "reply")
    if any(k in mtype for k in button_types):
        return True

    # fallback: ข้อความสั้น ๆ จากฝั่ง user มักเป็นปุ่มที่กดมา
    if len(raw) <= 40:
        return True
    return False
...
.badge.button,.mini.button{background:#17303f;color:#8fe0ff}
...
.chat-card::before{content:'';position:absolute;left:0;top:0;bottom:0;width:5px;background:#2a2d36}
.chat-card.paid::before{background:#c9a227}
.chat-card.pending::before{background:#4ea1ff}
.chat-card.active::before{background:#30c46c}
.chat-card.drop::before{background:#a56b7b}
.chat-card.idle::before{background:#666}
...
.kind.button{background:#17303f;color:#8fe0ff}
            ...
            (
                ...
                <div class="chat-card {user_status}">
                    <a href="{href}">
                        ...
                        <div class="badges">
                            <span class="badge button">BUTTON {stats['button']}</span>
...
def chat(chat_id: int, limit: int = 1200, kind: str = "all"):
    ...
    for r in rows:
        ...
        counts[event_kind] += 1
        if _looks_like_button_press(r["text"], r["msg_type"], r["direction"]):
            button_counter[_normalize_button_label(r["text"])] += 1
    ...
    button_items = "".join(
        f"<div class='kv'><span>{escape(label)}</span><strong>{count} ครั้ง</strong></div>"
        for label, count in button_counter.most_common(20)
    )
    button_summary = "".join(
        f"<span class='mini cmd'>{escape(label)} ×{count}</span>"
        for label, count in button_counter.most_common(8)
    )
    button_panel = (
        f"<div class='msg-card'><div class='section-title'>กดปุ่ม / ข้อความสั้นที่ user ส่งมา</div>{button_items}</div>"
        if button_items else ""
    )

    summary = (
        f"<div class='summary'>"
        f"<span class='status-pill {user_status}'>{status_label}</span>"
        f"<span class='mini button'>BUTTON {counts['button']}</span>"
